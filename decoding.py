...
def load_best_ckpt(model_dir, reverse=False):
    ...
    ckpt = torch.load(os.path.join(model_dir, 'ckpt/{}'.format(ckpts[0])))['state_dict']
    return ckpt
...
class Extractor(object):
    def __init__(self, ext_dir, max_ext=5, cuda=True):
        ...
        word2id = pkl.load(open(os.path.join(ext_dir, 'vocab.pkl'), 'rb'))
        self._device = torch.device('cuda' if cuda else 'cpu')
...
class RLExtractor(object):
    def __init__(self, ext_dir, cuda=True):
        ...
        word2id = pkl.load(open(os.path.join(ext_dir, 'agent_vocab.pkl'), 'rb'))
